...
public partial class PMBattleGUI : Control
{
    ...
    [Signal]
    public delegate void PlayerFinishedCommandInputEventHandler(Godot.Collections.Array<CombatEventData> data);
    ...
    public override void _Process(double delta)
    {
        if(active){
            base._Process(delta);
            var returnedAbility = currentMenu.HandleInput(ReadInput(), playersInQuestion[abilitiesQueued.Count()], parentBattle);
            if(returnedAbility != null){
                abilitiesQueued[abilitiesQueued.Count()] = returnedAbility.GetEventData();
                if(abilitiesQueued.Count() == playersInQuestion.Count){//When we have all of our abilities, we emit the signal
                    var output = new Godot.Collections.Array<CombatEventData>();
                    foreach(CombatEventData etnry in abilitiesQueued) output.Add(etnry);
                    EmitSignal(nameof(PlayerFinishedCommandInputEventHandler), output);
                    this.active = false;
                }
            }
        }
    ...
    //Due to Godot Signals not accepting custom object types, 
/*    public CombatEventData[] PickUpQueuedActions(){
    ...
    public MenuInput ReadInput(){
        ...
        if(Input.IsActionJustPressed("ui_select")){ return MenuInput.Select; }
